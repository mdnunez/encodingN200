...
%  08/27/18        Michael Nunez        Save out average Laplacian ERP
%  08/28/18        Michael Nunez          Comment out plotting code
    ...
    for sub = 1:length(subjects),
        for ses = 1:nses,
            ...
            if exist(tempsvdloc)==2,
                ...
                for cond = 1:3,
                    this_svd = svddata.(sprintf('svd_rint_%s',conds{cond}));
                    temperp = this_svd.erpinput;
                    % Calculate spline interpolated data
                    chanpos = eeg.hm.Electrode.CoordOnSphere;
                    splinemat = splineinterp(.1, chanpos, chanpos(goodchans,:));
                    ERPprojected = temperp*splinemat;
                    ERPprojected(:,eeg.hm.Electrode.NoInterp) = NaN;
                    scalperp(trackscalp, :,:) = ERPprojected;
                    trackscalp = trackscalp + 1
                end
            else
                fprintf('Did not find file %s \n',tempsvdloc);
...
% laperp = zeros(size(scalperp));
% for n=1:size(scalperp,1),
%     laperp(n,:,1:128) = squeeze(scalperp(n,:,goodchan))*Electrode.lap';
% end

% fprintf('Plotting the mean spline-Laplacian ERP...\n');
% figure;
% plotx(-99:1000,squeeze(mean(laperp,1)));

% save('ERPLap.mat','laperp');

% %Plot the ERP Laplacian with Cort Horton's function
% fprintf('Plotting the ERP Laplacian at 76 milliseconds on a 2D scalp...\n');
% figure; hortontopo(-squeeze(mean(laperp(:,176,:),1)),EGINN128,'channumbers',0,'drawelectrodes',0,...
%     'chanfontsize',8,'cmap','jet','badchans',[129]);

% %Plot the ERP Laplacian with Cort Horton's function
% fprintf('Plotting the ERP Laplacian at 167 milliseconds on a 2D scalp...\n');
% figure; hortontopo(squeeze(mean(laperp(:,279,:),1)),EGINN128,'channumbers',0,'drawelectrodes',0,...
%     'chanfontsize',8,'cmap','jet','badchans',[129]);

% %Plot the ERP Laplacian on a 3D surface
% fprintf('Plotting the Laplacian response on a 3D scalp...\n');
% erpCSD = squeeze(mean(laperp(:,134,1:128),1))'*Electrode.spline3D';
...
% %Plot the ERP Laplacian on a 3D surface
% fprintf('Plotting the Laplacian response on a 3D scalp...\n');
% erpCSD = squeeze(mean(laperp(:,176,1:128),1))'*Electrode.spline3D';
...
% %Plot the ERP Laplacian on a 3D surface
% fprintf('Plotting the Laplacian response on a 3D scalp...\n');
% erpCSD = squeeze(mean(laperp(:,279,1:128),1))'*Electrode.spline3D';
...
% randsamp = randperm(size(laperp,1),9);
...
%     figure(eegplot);
%     subplot(3,3,n);
%     hortontopo(squeeze(laperp(randsamp(n),176,:)),EGINN128,'channumbers',0,'drawelectrodes',0,...
%     'chanfontsize',8,'cmap','jet','badchans',[Electrode.badchan.eeg 129]);
%     figure(lapplot);
%     subplot(3,3,n);
%     hortontopo(squeeze(laperp(randsamp(n),279,:)),EGINN128,'channumbers',0,'drawelectrodes',0,...
%     'chanfontsize',8,'cmap','jet','badchans',[129]);
% end

# pdm5b_predictionmodel7.py - Finds and evaluates posterior predictive distributions
#                           for single-trial N200 peak-latencies, RTs, and accuracies
#
...
# ====         ================                       ======================
# 08/07/18      Michael Nunez                            Original code
#
#
...
vetsub_samps = combinechains(samples['vetsub'])
n200trial_samps = combinechains(samples['n200trialsd'])
rmrsub_samps = combinechains(samples['rmrsub'])
...
npossamps = np.shape(vetsub_samps)[-1]
ndraws = 1000
nses = 49
nconds = 3
simulated_n200 = np.zeros((nconds,nses,ndraws))
...
for d in draw:
    ...
    for s in range(0,nses):
        for c in range(0,nconds):
            simulated_n200[c,s,drawcount] = truncnorm.rvs(.151,.274,loc=vetsub_samps[c,s,d],scale=n200trial_samps[0,d])
            traintrials = np.where( (condition==(c+1)) & (sessioncount==(s+1)) )[0]
            testtrials = np.where( (condition_test==(c+1)) & (sessioncount_test==(s+1)) )[0]
            #####
            tempdiffsamps = np.empty(np.size(traintrials))
            tertrial = np.empty(np.size(traintrials))
            trainindex = 0
            for t in traintrials:
                tertrial[trainindex] = n200lat[t] + rmrsub_samps[c,s,d]
                tempdiffsamps[trainindex] = simuldiff(N=1,Ter=tertrial[trainindex],Alpha=alphasub_samps[c,s,d],Nu=deltasub_samps[c,s,d],Eta=0)
                trainindex += 1
            known_lapse_trials = np.where(np.abs(y[traintrials]) < tertrial)[0] #Known lapse trials
            tempdiffsamps[known_lapse_trials] = np.random.uniform(-maxrt[c,s],maxrt[c,s],size=np.size(known_lapse_trials)) # Lapse process
            known_lapse_proportion = np.size(known_lapse_trials)/np.size(y) #Proportion of known lapse trials
            estimate_lapse_proportion = probsub_samps[c,s,1,d] #Estimated proportion of lapse trials
            if estimate_lapse_proportion > known_lapse_proportion:
                probable_nonlapse_trials = np.where(np.abs(y[traintrials]) > tertrial)[0]
                remaining_lapse_proportion = estimate_lapse_proportion - known_lapse_proportion
            else:
                remaining_lapse_porportion = 0
            lapsetrials = np.random.choice(np.size(probable_nonlapse_trials),np.round(np.size(probable_nonlapse_trials)*remaining_lapse_proportion),replace=False)
            ...
            #####
            tempdiffsamps_test = np.empty(np.size(testtrials))
            tertrial_test = np.empty(np.size(testtrials))
            testindex = 0
            for t in testtrials:
                tertrial_test[testindex] = n200lat_test[t] + rmrsub_samps[c,s,d]
                tempdiffsamps_test[testindex] = simuldiff(N=1,Ter=tertrial_test[testindex],Alpha=alphasub_samps[c,s,d],Nu=deltasub_samps[c,s,d],Eta=0)
                testindex += 1
...
sim_n200_means = np.zeros((nconds,nses))
train_n200_means = np.zeros((nconds,nses))
test_n200_means = np.zeros((nconds,nses))
sim_n200_stds = np.zeros((nconds,nses))
train_n200_stds = np.zeros((nconds,nses))
test_n200_stds = np.zeros((nconds,nses))
...
for s in range(0,nses):
    for c in range(0,nconds):
        sim_n200_means[c,s] = np.mean(simulated_n200[c,s,:])
        train_n200_means[c,s] = np.mean(n200lat[(condition==(c+1)) & (sessioncount==(s+1))])
        test_n200_means[c,s] = np.mean(n200lat_test[(condition_test==(c+1)) & (sessioncount_test==(s+1))])
        sim_n200_stds[c,s] = np.std(simulated_n200[c,s,:])
        train_n200_stds[c,s] = np.std(n200lat[(condition==(c+1)) & (sessioncount==(s+1))])
        test_n200_stds[c,s] = np.std(n200lat_test[(condition_test==(c+1)) & (sessioncount_test==(s+1))])
...
# Note that the hierarchical model does not constrain single-trial N200 latencies
insample_n200_means = rsquaredpred(train_n200_means,sim_n200_means)
print "The in-sample prediction of single-trial N200 means explains %.3f %% of the variance" % (insample_n200_means*100)
outsample_n200_means = rsquaredpred(test_n200_means,sim_n200_means)
print "The out-of-sample prediction of single-trial N200 means explains %.3f %% of the variance" % (outsample_n200_means*100)

insample_n200_stds = rsquaredpred(train_n200_stds,sim_n200_stds)
print "The in-sample prediction of single-trial N200 stds explains %.3f %% of the variance" % (insample_n200_stds*100)
outsample_n200_stds = rsquaredpred(test_n200_stds,sim_n200_stds)
print "The out-of-sample prediction of single-trial N200 stds explains %.3f %% of the variance" % (outsample_n200_stds*100)

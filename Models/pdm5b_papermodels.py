...
# 08/10/18      Michael Nunez                       Creation of 4parameter_lapse_alt
...
# Random effects of single-trial N200 latency on all parameters, split by experiment
#  Properly accounting for lapse trials
jagsmodels['all_n200lat_random_lapse'] = '''
model {
	##########
	#Fixed Parameters
	beta <- .5
...
# Random effects of single-trial N200 latency on ter, split by experiment
#  Properly accounting for lapse trials
jagsmodels['ter_n200lat_random_lapse'] = '''
model {
	##########
	#Fixed Parameters
	beta <- .5
...
# 4 Parameter Model that describes observed N200 peak-latencies on single-trials, split by experiment
#  Properly accounting for lapse trials
#Note that this is actually a 5 parameter model and therefore not restrictive on N200 peak-latencies
...
model {
	...
	for (k in 1:nconds) {
		for (e in 1:nexps) {
			...
			vetsdcond[e,k] ~ dnorm(.05, pow(.025,-2))
		...
		for (ses in 1:nses) {
		    ...
		    #EEGsession-level VET variability
			vetsdsub[k,ses] ~ dnorm(vetsdcond[experiment[ses]+1,k],
				pow(vetsdsubsd, -2))
	...
	for (i in 1:N) {
		...
		dt[i] ~ dwiener(alphasub[condition[i],EEGsession[i]],
        0,
        beta,
        deltasub[condition[i],EEGsession[i]])

		vetsub[i] <- abs(y[i]) -  abs(dt[i]) - rmrsub[condition[i],EEGsession[i]]

		#Note that N200 latencies are censored between 150 and 274 ms
		n200lat[i] ~ dnorm(vetsub[i],
			pow(vetsdsub[condition[i],EEGsession[i]],-2))T(.151,.274)

        # Log density for DDM process
        ld_comp[i, 1] <- dlogwiener(y[i] - ifelse((y[i] > 0),1,-1)*n200lat[i],alphasub[condition[i],EEGsession[i]],
        rmrsub[condition[i],EEGsession[i]],
        beta,
        deltasub[condition[i],EEGsession[i]])

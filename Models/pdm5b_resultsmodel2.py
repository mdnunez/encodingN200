# pdm5b_resultsmodel1.py - Track model builds of model type 'request2'
#
#
...
# ====         ================                       ======================
# 01/29/18       Michael Nunez              Converted from pdm5b_resultsmodel1.py
# 01/30/18       Michael Nunez             Report credible intervals for base effects
#
# Imports
import os
import numpy as np
...
# Plot figures
jagsmodel = 'jagsmodel_all_n1lat_request2Jan_26_18_14_32.mat'
fontsize = 18
...
plt.figure()
jellyfish(samples['n1cond'])
plt.title('Condition-level N1 latencies', fontsize=fontsize)

# Calculate Bayes Factors
possamps = samples['n1gammacond'][0, :, [2, 1, 0]]
# Number of chains
nchains = possamps.shape[-1]
# Number of samples per chain
nsamps = possamps.shape[-2]
...
jellyfish(samples['n1gammacond'][0, :, [2, 1, 0]])

plt.plot([1, 1], [0, 1.5], 'r--', linewidth=2)
plt.plot([0, 0], [0, 1.5], 'b--', linewidth=2)
plt.plot([1, 1], [1.5, 7], 'b--', linewidth=2)
plt.plot([0, 0], [1.5, 7], 'r--', linewidth=2)
plt.xlim((-3, 4))
# plt.title('Condition-level N1 effects', fontsize=fontsize)
yticks = axes1.get_yticks().tolist()
yticks[0] = ''
yticks[1] = 'Base effect'
yticks[2] = 'Med Noise'
yticks[3] = 'High Noise'
yticks[4] = 'Exp 2'
yticks[5] = 'Med. Noise*Exp2'
yticks[6] = 'High Noise*Exp2'
axes1.set_yticklabels(yticks)
plt.xlabel(
    'Effect of N1 latency on non-decision time (ms increase per ms increase in N1 latency)', fontsize=fontsize)
axes1.set_yticklabels(yticks)
plt.xlabel(
    'Moderator effects of N1 latency on non-decision time (additional ms increase per ms increase in N1 latency)', fontsize=fontsize)
axes1.tick_params(axis='both', which='major', labelsize=fontsize)
# Calculate Bayes Factors for the null model using Savage-Dickey density
# ratio
bf = dict()
for k in range(0, 6):
    ...
    if k == 0:
        ...
        bf[k] = kde(1) / denom
        plt.annotate('BF: %3.2f' %
                     bf[k], (3.25, k + .95), fontsize=fontsize)
    else:
        denom = stats.norm.pdf(0, loc=0, scale=1)
        bf[k] = kde(0) / denom
        plt.annotate('BF0: %3.2f' %
                     bf[k], (3.25, k + .95), fontsize=fontsize)
...
if (len(samples['n1gammacond'].shape) > 4):
    ...
    plt.title('Moderator effects of N1 latency on drift-rate', fontsize=fontsize)
    plt.figure()
    jellyfish(samples['n1gammacond'][2])
    plt.title('Moderator effects of N1 latency on boundary separation',
              fontsize=fontsize)
...
percentiles1 = stats.scoreatpercentile(np.reshape(samples['n1gammacond'][0,0,2,:],(nchains * nsamps)), (50, 2.5, 97.5))
...
percentiles2 = stats.scoreatpercentile(np.reshape(samples['n1gammacond'][1,0,2,:],(nchains * nsamps)), (50, 2.5, 97.5))
...
percentiles3 = stats.scoreatpercentile(np.reshape(samples['n1gammacond'][2,0,2,:],(nchains * nsamps)), (50, 2.5, 97.5))

...
if (len(samples['n1gammacond'].shape) > 4):
    ...
    plt.title('Effect of N1 latency on drift-rate', fontsize=fontsize)
    plt.figure()
    jellyfish(samples['n1gammacond'][2])
    plt.title('Effect of N1 latency on boundary separation',
              fontsize=fontsize)
...
print 'The median of the posterior of the effect of N200 on non-decision time is %.3f, 95%% CI: [%.3f, %.3f]' % (percentiles1[0],percentiles1[1],percentiles1[2])

percentiles2 = stats.scoreatpercentile(np.reshape(samples['n1gammault'][0,1,:],(nchains * nsamps)), (50, 2.5, 97.5))
print 'The median of the posterior of the effect of N200 on drift rate is %.3f, 95%% CI: [%.3f, %.3f]' % (percentiles2[0],percentiles2[1],percentiles2[2])

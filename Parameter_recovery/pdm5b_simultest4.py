# pdm5b_simultest4.py - Testing JAGS fits of Joint models of single-trial N200 latencies
#                       and response time and accuracy
#
...
# ====         ================                       ======================
# 08/10/18      Michael Nunez              Converted from pdm5b_simultest3.py
...
# rmrsub = np.random.uniform(.15, .275, size=(nsims, nsubs)) # Uniform from .15 to .275 seconds
...
#         mindwanderx = np.random.randint(low=0,high=2,size=ntrials)
...
modelfile = 'simultrialparams4.jags'
f = open(modelfile, 'w')
f.write(tojags)
f.close()

# Track these variables
trackvars = ['deltasubsd', 'alphasubsd','rmrsubsd', 'vetsdsubsd',
             'deltacond', 'alphacond', 'rmrcond', 'vetsdcond',
             'deltasub', 'alphasub', 'rmrsub', 'vetsdsub',
             'probsub']


# #Temporary pause
# waithours = 12;
# print 'Pausing for %d hours to wait for another computation to finish...' % waithours
# time.sleep(waithours*60*60)
...
for n in range(0, nsims):
    ...
    for s in range(1, nsubs + 1):
        ...
        n200lat[indextrack:(indextrack+subn)] = genparam['n200'][n, s - 1, whereindx]
    ...
    for c in range(0, nchains):
        chaininit = {
            'rmrsub': np.random.uniform(.1, .3, size=nsubs),
            'vetsdsub': np.random.uniform(0, .1, size=nsubs),
            'alphasub': np.random.uniform(.5, 2., size=nsubs),
            'deltasub': np.random.uniform(-4., 4., size=nsubs),
            'rmrcond': np.random.uniform(.1, .3),
            'vetsdcond': np.random.uniform(0, .1),
            ...
            'vetsdsubsd': np.random.uniform(.01, .1),
            'probsub': np.stack((np.zeros((nsubs)),np.ones((nsubs))),axis=1)
        }
    ...
    threaded = pyjags.Model(file=modelfile, init=initials,
                            data=dict(y=y, N=N, n200lat=n200lat, nsubs=nsubs, maxrt=maxrt,
    ...
    savestring = ('modelfits/trialparam4_test_model%i.mat') % (n + 1)

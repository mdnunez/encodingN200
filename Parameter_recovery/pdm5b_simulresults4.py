# pdm5b_simulresults3.py - Evaluates simulation results of 
#                       Joint models of single-trial N200 latencies
...
# ====         ================                       ======================
# 08/10/18      Michael Nunez           Converted from pdm5b_simulresults3.py

#To do:
# 1) Fix convergence issue in some simulated participants
...
linearmodel = 'modelfits/trialparam4_test_model%i.mat'
...
for m in range(1, nsims + 1):
    ...
    tempbetas0 = np.polyfit(
        np.mean(genparam['n200'][(m - 1), :,:],axis=1).T, rtpercentiles[(m - 1), :], deg=1)
    ...
    plt.figure()
    recovery(samples[m]['rmrsub'][:] * 1000,
             genparam['rmrsub'][(m - 1), :].T * 1000)
    plt.title('Recovery of residual motor response times from model fit, Model %i' %
              (m), fontsize=16)
    plt.xlabel('Real residual motor response times (ms)', fontsize=16)
    plt.ylabel('Residual motor response time Posteriors (ms)', fontsize=16)
    tempbetas1 = np.polyfit(genparam['rmrsub'][(m - 1), :] * 1000, np.median(
        samples[m]['rmrsub'][:].reshape((nsubs, 6000)), axis=1) * 1000, deg=1)
...
plt.title('Changing slopes of real RMR versus posterior medians', fontsize=16)
plt.xlabel('Simulation # (Increasing wind wandering)', fontsize=16)
plt.ylabel('Beta value', fontsize=16)